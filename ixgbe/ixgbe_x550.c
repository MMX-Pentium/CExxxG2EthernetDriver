...
/*
 * The CE100G2/NCA-2011Z connects both X553 KR functions directly to the
 * 88E6190 CPU ports using fixed 2500BASE-X. Its NVM advertises only 2.5G,
 * but the generic 2.5G path deliberately preserves the existing PCS state.
 * On Ubuntu that state has been observed as 1G/down after reset.
 *
 * Keep the workaround board-specific: this source also replaces ixgbe for
 * unrelated Intel adapters while the package is installed.
 */
static const struct dmi_system_id ixgbe_ce100g2_dmi_table[] = {
...
bool ixgbe_is_ce100g2(void)
{
	return dmi_check_system(ixgbe_ce100g2_dmi_table);
}
...
{
	u32 reg_val, old_reg_val;
	s32 status;

	/* Upstream leaves the PCS untouched for 2.5G. On the CE100G2, make
	 * the fixed 2500BASE-X board wiring explicit after every X553 reset.
	 */
	if (hw->phy.autoneg_advertised & IXGBE_LINK_SPEED_2_5GB_FULL) {
		if (hw->mac.type != ixgbe_mac_x550em_a || !ixgbe_is_ce100g2())
			return 0;
		...
		reg_val &= ~IXGBE_KRM_PMD_FLX_MASK_ST20_AN37_EN;
		reg_val &= ~IXGBE_KRM_PMD_FLX_MASK_ST20_SGMII_EN;
		reg_val |= IXGBE_KRM_PMD_FLX_MASK_ST20_SPEED_2_5G;
		...
		netdev_info(ixgbe_hw_to_netdev(hw),
			    "CE100G2 fixed 2.5G PCS: PMD FLX %#010x -> %#010x\n",
			    old_reg_val, reg_val);
