...
MODULE_VERSION("6.8.0-100-ce100g2-0.1.10");
...
static int ixgbe_non_sfp_link_config(struct ixgbe_hw *hw)
{
	...
	if (ixgbe_is_ce100g2() &&
		...
		/* The NVM reports only 2.5G for these fixed switch uplinks, but
		 * autoneg_advertised can retain the reset PCS value (observed as
		 * 1G). Do not let that stale value bypass capability selection.
		 */
		speed = IXGBE_LINK_SPEED_2_5GB_FULL;
		netdev_info(adapter->netdev,
			    "CE100G2 requesting fixed 2.5G X553 link setup\n");
	} else if (!speed && hw->mac.ops.get_link_capabilities) {
...
#define IXGBE_ESP_FEATURES	(NETIF_F_HW_ESP | \
	...
	/* Make sure the EEPROM is good. Both X553 functions share the NVM;
	 * on the CE100G2 their concurrent boot probes can make one validation
	 * fail transiently even though a later bind succeeds.
	 */
	err = hw->eeprom.ops.validate_checksum(hw, NULL);
	if (err < 0 && ixgbe_is_ce100g2()) {
		for (i = 1; i <= 5 && err < 0; i++) {
			msleep(200);
			err = hw->eeprom.ops.validate_checksum(hw, NULL);
		}
		if (!err)
			e_dev_warn("CE100G2 EEPROM checksum recovered after %d retries\n",
				   i - 1);
	}
	if (err < 0) {
